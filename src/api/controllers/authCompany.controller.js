...
export const forgotPassword = (req, res) => {
  ...
  db.query(q, [email], (err, data) => {
    if (!data?.length) {
      ...
      const token = jwt.sign({ id: data[0].id }, process.env.MY_SECRET, { expiresIn: '60s' }); // token tồn tại trong 1 phút
      ...
      const url = `${process.env.URL_REACT}/tao-moi-mat-khau/${data[0].id}/${token}?type=nha-tuyen-dung`;
...
export const resetPassword = (req, res) => {
  ...
  jwt.verify(token, process.env.MY_SECRET, (err, userInfo) => {
    if (err) {
      return res.status(401).json("Liên kết không đúng hoặc đã quá hạn. Vui lòng xác nhận lại email.");
    } else {
